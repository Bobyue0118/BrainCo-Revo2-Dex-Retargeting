...
# Try to import MediaPipe with legacy API first, then new API
try:
...
class Revo2HandRetargeting:
    ...
    def __init__(self, urdf_path: str, hand_side: str = "right"):
        ...
        # Initialize MediaPipe Hand detector (will be done in process_video for new API)
        if USE_LEGACY_API:
    ...
    def _draw_landmarks_on_image(self, image, hand_landmarks):
        """Draw hand landmarks on image (compatible with both APIs)."""
    ...
    def process_video(self, video_path: str, output_path: str = None, 
        ...
        # Initialize MediaPipe Hands based on API version
        if self.use_legacy_api:
            # Legacy API
            with self.mp_hands.Hands(
                ...
                trajectory = self._process_frames_legacy(
                    cap, hands, out, trajectory, width, height, fps, total_frames
                )
        else:
            # New API - process with updated method
            trajectory = self._process_frames_new_api(
                cap, out, trajectory, width, height, fps, total_frames
            )
